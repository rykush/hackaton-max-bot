...
const Dictionary = () => {
  ...
  const renderAnswer = (answer) => {
    if (Array.isArray(answer)) {
      const linkItems = answer.filter(item => isUrl(item.text));
      const hasMultipleLinks = linkItems.length > 1;
      
      return (
        <div className={`answer-content ${hasMultipleLinks ? 'has-multiple-links' : ''}`}>
          {answer.map((item, index) => (
            <div 
              key={index} 
              className={`answer-item ${isUrl(item.text) ? 'is-link' : ''}`}
            >
              {item.letter && <span className="answer-letter">{item.letter}</span>}
              {isUrl(item.text) ? (
                <Button
                  appearance="themed"
                  asChild
                ...
                >
                  <a
                    ...
                    target="_blank"
                  >
                    {hasMultipleLinks ? `Открыть сайт ${linkItems.indexOf(item) + 1}` : 'Открыть сайт'}
                  </a>
                </Button>
              ) : (
      ...
      return (
        <div className="answer-content">
          {paragraphs.map((paragraph, index) => (
            <p key={index} className="answer-text">{paragraph}</p>
          ))}
        </div>
      );
    }
      ...
      questions: [
        ...
        },
        {
          ...
          question: 'Как принять участие в публичных консультациях?',
            ...
            {
              letter: '',
              text: 'Необходимо авторизоваться на портале для публичных обсуждений (авторизация на https://regulation.gov.ru/ осуществляется через ЕСИА), зайти в карточку интересующего Вас проекта, после изучения проекта ответить на вопросы по нему либо прикрепить файл с отзывом'
            },
            {
              letter: '',
              text: 'https://regulation.gov.ru/'
            },
          ]
        },
        {
      ...
      questions: [
        ...
        },
        {
          ...
          question: 'Чем обязательные требования отличаются от иных обязанностей и запретов?',
            ...
            {
              letter: '',
              text: 'Обязательные требования отличает наличие процедуры оценки соблюдения и санкций (мер ответственности/ допуск к деятельности). Два необходимых условия установления обязательных требований: '
            },
            {
              letter: '',
              text: '1. наличие риска причинения вреда (ущерба) охраняемым законом ценностям, на устранение которого направлено установление обязательных требований'
            },
            {
              letter: '',
              text: '2. возможность и достаточность установления обязательных требований в качестве мер защиты охраняемых законом ценностей.'
            },
          ]
        },
        {
      ...
      questions: [
        ...
        },
        {
          ...
          answer: [
            {
              letter: '',
              text: 'Проекты НПА субъектов РФ:'
            },
            {
              letter: '',
              text: '1) устанавливающие новые или изменяющие ранее предусмотренные нормативными правовыми актами субъектов РФ обязательные требования, связанные с осуществлением предпринимательской и иной экономической деятельности, оценка соблюдения которых осуществляется в рамках государственного контроля (надзора), привлечения к административной ответственности, предоставления лицензий и иных разрешений, аккредитации, оценки соответствия продукции, иных форм оценок и экспертиз;'
            },
            {
              letter: '',
              text: '2) устанавливающие новые или изменяющие ранее предусмотренные нормативными правовыми актами субъектов РФ обязанности и запреты для субъектов предпринимательской и инвестиционной деятельности;'
            },
            {
              letter: '',
              text: '3) устанавливающие или изменяющие ответственность за нарушение нормативных правовых актов субъектов РФ, затрагивающих вопросы осуществления предпринимательской и иной экономической деятельности.'
            },
            {
              letter: '',
              text: 'За исключением:'
            ...
            {
              letter: '',
              text: '1) проектов законов субъектов РФ об установлении, о введении в действие или прекращении действия налогов (сборов), об изменении налоговых ставок (ставок сборов), порядка и срока уплаты налогов (сборов), установлении (отмене) налоговых льгот (льгот по сборам) и (или) оснований и порядка их применения;'
            },
            {
              letter: '',
              text: '2) проектов законов субъектов РФ, регулирующих бюджетные отношения;'
            },
            {
              letter: '',
              text: '3) проектов нормативных правовых актов субъектов РФ:'
            },
            {
              letter: '',
      ...
      questions: [
        ...
        },
        {
          ...
          answer: [
            {
              letter: '',
              text: 'Да, Вы можете обратиться к Онлайн калькулятору регулирующего воздействия https://calculator-rv.ru/user/login/'
            },
            {
              letter: '',
              text: 'https://calculator-rv.ru/user/login/'
            }
          ]
        },
        {
          ...
          answer: [
            {
              letter: '',
              text: 'Для расчета издержек используйте методику стандартных издержек (МСИ). Основные шаги:'
            },
            {
              letter: '',
              text: '1) Определите все новые обязанности, которые вводит проект (получение документов, заполнение форм, проведение экспертиз и т.д.).'
            },
            {
              letter: '',
              text: '2) Для каждой обязанности рассчитайте: время на выполнение × стоимость часа работы специалиста + прямые затраты (госпошлины, услуги экспертов).'
            },
            {
              letter: '',
              text: '3) Умножьте на количество субъектов, которых коснется требование, и частоту выполнения (разово, ежегодно и т.д.).'
            },
            {
              letter: '',
              text: '4) Суммируйте все издержки. Полученная цифра — это ваш аргумент в пользу смягчения или отмены избыточных требований.'
            }
          ]
        },
        {
      ...
      questions: [
        {
          ...
          answer: [
            {
              letter: '',
              text: 'Федеральный портал: https://regulation.gov.ru/ - все федеральные проекты НПА. Региональные порталы: ссылки на них размещены на федеральном портале в разделе "Субъекты РФ". Telegram-канал "Регуляторная гильотина и ОРВ": https://t.me/reguliarnayaguillotina - новости, разъяснения, анонсы важных обсуждений.'
            },
            {
              letter: '',
              text: 'https://regulation.gov.ru'
        ...
        },
        {
          ...
          answer: [
            {
              letter: '',
              text: 'Официальный сайт Минэкономразвития России: https://www.economy.gov.ru/ - раздел "Оценка регулирующего воздействия". Портал https://regulation.gov.ru - раздел "Методические материалы". Онлайн-калькулятор РВ: https://calculator-rv.ru/ - инструкции и примеры расчетов.'
            },
            {
              letter: '',
              text: 'https://www.economy.gov.ru/'
            },
            {
              letter: '',
              text: 'https://regulation.gov.ru'
      ...
      questions: [
        {
          ...
          answer: [
            {
              letter: '',
              text: 'Для повышения шансов на учет предложений:'
            },
            {
              letter: '',
              text: '1) Подавайте предложения как можно раньше в период публичных консультаций.'
            },
            {
              letter: '',
              text: '2) Объединяйтесь с коллегами по отрасли для подачи коллективных предложений.'
            },
            {
              letter: '',
              text: '3) Подкрепляйте предложения конкретными расчетами и примерами.'
            },
            {
              letter: '',
              text: '4) Предлагайте конструктивные альтернативы, а не только критикуйте.'
            },
            {
              letter: '',
              text: '5) Участвуйте в очных обсуждениях, если они проводятся.'
            }
          ]
        },
        {
...
answer: [
            {
              letter: '',
              text: 'Если ваши предложения не учтены:'
            },
            {
              letter: '',
              text: '1) Изучите обоснование отказа в сводке предложений.'
            },
            {
              letter: '',
              text: '2) Если обоснование неубедительно, направьте повторное обращение с дополнительными аргументами.'
            },
            {
              letter: '',
              text: '3) Обратитесь в Минэкономразвития России для проверки качества проведения ОРВ.'
            },
            {
              letter: '',
              text: '4) После принятия акта можете инициировать его экспертизу или оспорить в суде, если он нарушает ваши права.'
            }
          ]
        }
      ]
